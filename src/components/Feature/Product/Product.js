...
const Product = ({ products }) => {
  return (
    <section className="mt-10">
      <h1 className="text-3xl font-bold text-center text-gray-800 mb-8">Most Popular</h1>
      <div className="flex flex-wrap justify-center gap-8">
        {products &&
          products.map((product) => (
            <Link to={`/${product.title}/${product.id}`} key={product.id} className="w-full sm:w-72">
              <div className="bg-white border border-gray-200 rounded-lg shadow-lg transition transform hover:scale-105 hover:shadow-2xl">
                <img className="w-full h-48 object-cover rounded-t-lg" src={product.image} alt={product.title} />
                <div className="p-5">
                  <h5 className="mb-2 text-xl font-bold tracking-tight text-gray-900">{product.title}</h5>
